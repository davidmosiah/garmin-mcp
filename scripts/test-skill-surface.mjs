...
assert.match(skill, /call garmin_connection_status/);
...
const result = await run(["call", "garmin_connection_status", "--json", "{}"]);
...
console.log(JSON.stringify({ ok: true, suite: "skill-surface", tool: "garmin_connection_status", files: pkg.files }, null, 2));
